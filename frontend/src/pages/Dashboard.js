...
//     api.importance().then(r => setImportance(r.importance));
//   }, []);
...
//   useEffect(() => {
//     if (!selected) return;
//     api.vitals(selected.id).then(r => setVitals(r.series));
//     // build feature vector for explain call (use patient's "features" if present, else assume patient object is already features)
//     const feat = selected.features || selected;
